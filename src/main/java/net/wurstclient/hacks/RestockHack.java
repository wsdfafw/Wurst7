...
public final class RestockHack extends Hack implements UpdateListener
{
	private ItemListSetting items = new ItemListSetting("Items",
		"Item to be restocked.", "minecraft:minecart");
	private final CheckboxSetting currentSlot = new CheckboxSetting("Current slot", "Always restock in the current slot.", false);
	private final CheckboxSetting repairMode = new CheckboxSetting("Tools repair mode", "Swap out tools that are about to break.", false);
	...
	private boolean itemEqual(String itemName, ItemStack stack) {
		if (repairMode.isChecked() && stack.isDamageable() && isTooDamaged(stack)) {
			return false;
		}
	...
	public void onUpdate() {
		...
		for (String itemName : items.getItemNames()) {
			...
			boolean wrongItem = restockStack.isEmpty() || !itemEqual(itemName, restockStack);
			if (!wrongItem && restockStack.getCount() >= Math.min(2, restockStack.getMaxCount())) {
				return;
			}

			List<Integer> searchResult = searchSlotsWithItem(itemName, 0, 36, restockSlot);
			for (int itemIndex : searchResult) {
				...
				if (restockStack.getCount() >= restockStack.getMaxCount()) break;
			}
		...
		ItemStack restockStack = MC.player.getInventory().getStack(restockSlot);
		if (repairMode.isChecked() && restockStack.isDamageable() && isTooDamaged(restockStack)) {
			for (int i = 36-1; i > 9-1; i--) {
				ItemStack stack = MC.player.getInventory().getStack(i);
				if (stack.isEmpty() || !stack.isDamageable()) {
					IMC.getInteractionManager().windowClick_SWAP(i, restockSlot);
					break;
				}
			}
		}
	...
	private List<Integer> searchSlotsWithItem(String itemName, int start, int end, int skip) {
